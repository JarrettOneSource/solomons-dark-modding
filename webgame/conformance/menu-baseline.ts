...
export async function verifyMenuBaseline(
  ...
  if (root.schema !== "solomon-dark-menu-baseline-v1") {
  ...
  if (root.baseline_snapshot_count !== MENU_BASELINE_COUNT || root.pending_shellfix_count !== MENU_BASELINE_COUNT) {
    throw new Error("menu baseline manifest census must remain exactly 28 snapshots and 28 pending fixtures");
  }
  ...
  if (snapshots.size !== MENU_BASELINE_COUNT || pending.size !== MENU_BASELINE_COUNT) {
    throw new Error("menu baseline manifest arrays must each enumerate exactly 28 fixtures");
  }
  const union = new Set([...snapshots.keys(), ...pending.keys()]);
  if (
    union.size !== canonicalFixtures.size
    || [...union].some((fixture) => !canonicalFixtures.has(fixture))
    || [...canonicalFixtures].some((fixture) => !snapshots.has(fixture) || !pending.has(fixture))
  ) {
    throw new Error("every G11 fixture must be both snapshot-matched and pending-enumerated");
  }
  for (const fixture of canonicalFixtures) {
    const snapshot = snapshots.get(fixture);
    const pendingEntry = pending.get(fixture);
    if (snapshot === undefined || pendingEntry === undefined) {
      throw new Error(`menu baseline verification lost canonical fixture ${fixture}`);
    }
    if (snapshot.corrective !== MENU_BASELINE_CORRECTIVE || pendingEntry.corrective !== MENU_BASELINE_CORRECTIVE) {
  ...
  return { snapshots, pendingShellfix: pending };
}
