...
void RecoverSteamSessionFromGameplayCongestion(
    ...
    const auto peer = g_session.peers.find(remote_steam_id);
    if (!g_session.initialized ||
        remote_steam_id == 0 ||
        peer == g_session.peers.end() ||
        !peer->second.authenticated) {
        ResetSteamGameplayPeerSendQueue(remote_steam_id);
        return;
    }

    Log(
        "Steam gameplay send congestion recovery. steam_id=" +
        std::to_string(remote_steam_id) +
        " duration_ms=" + std::to_string(duration_ms) +
        " queued_reliable=" +
        std::to_string(queued_reliable_packets) +
        " dropped_disposable=" +
        std::to_string(dropped_disposable_packets));

    if (!g_session.is_host &&
        remote_steam_id == g_session.host_steam_id) {
        RestartClientHostHandshake(
            remote_steam_id,
            "gameplay_send_congestion",
            true);
        return;
    }

    SuspendPeerForReauthentication(remote_steam_id);
    const bool route_reset =
        SteamCloseNetworkSession(remote_steam_id);
    ...
    Log(
        "Steam multiplayer reset saturated peer route. steam_id=" +
        std::to_string(remote_steam_id) +
