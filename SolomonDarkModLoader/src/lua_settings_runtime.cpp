...
bool RequiresHostSettingsBarrier(const LoadedLuaMod& mod) {
    return std::find(
        mod.descriptor.required_capabilities.begin(),
        mod.descriptor.required_capabilities.end(),
        "settings.list") != mod.descriptor.required_capabilities.end() &&
        std::any_of(
            mod.settings_declaration.entries.begin(),
            mod.settings_declaration.entries.end(),
            [](const ModSettingEntry& entry) {
                return entry.type != ModSettingType::Action &&
...
bool HaveCompleteReplicatedHostSettings(const LoadedLuaMod& mod) {
    ...
    for (const auto& entry : mod.settings_declaration.entries) {
        ...
        ModSettingValue value;
        if (!TryGetReplicatedValue(mod, entry, &value)) {
            return false;
        }
    }
    return true;
...
bool ShouldDeferLuaEntryForHostSettings(const LoadedLuaMod* mod) {
    return mod != nullptr &&
        ...
        RequiresHostSettingsBarrier(*mod) &&
...
void PollLuaSettingsReplicationChanges() {
    ...
    for (const auto& loaded : LoadedLuaModsStorage()) {
        ...
        if (mod->entry_script_deferred_for_host_settings) {
            if (!HaveCompleteReplicatedHostSettings(*mod)) {
                continue;
            }
            LogLuaMessage(
                *mod,
                "authoritative host settings ready before entry script; monotonic_ms=" +
                    std::to_string(static_cast<std::uint64_t>(GetTickCount64())));
            std::string load_error;
            if (!CreateLuaStateForMod(
                    mod,
                    mod->descriptor.entry_script_path,
                    &load_error)) {
                LogLuaMessage(
                    *mod,
            ...
            LogLuaMessage(
                *mod,
                "started deferred entry script after host settings; monotonic_ms=" +
                    std::to_string(static_cast<std::uint64_t>(GetTickCount64())));
...
#include "lua_settings_runtime_actions.inl"
