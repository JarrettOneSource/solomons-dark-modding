...
void FinishLuaExecRequest(
    ...
    if (request->completion) {
        try {
            ...
            Log("[lua] asynchronous exec completion threw an exception.");
        }
    }
...
bool CreateLuaStateForMod(
    ...
    const bool entry_script_loaded = ExecuteEntryScript(mod, entry_script_path, error_message);
...
void ShutdownLuaEngine() {
    ...
    std::unique_lock<std::mutex> lock(detail::LuaEngineMutex());
    if (!detail::LuaEngineInitializedFlag()) {
...
bool QueueLuaExecRequestAsync(
    ...
    LuaExecCompletion completion) {
    LuaExecResult admission_error;
    if (code.empty()) {
        admission_error.error = "No Lua code was provided.";
        if (completion) {
            completion(std::move(admission_error));
        }
        return false;
    }

    {
        std::scoped_lock lock(detail::LuaEngineMutex());
        if (detail::LuaEngineInitializedFlag()) {
            detail::EnqueueLuaExecRequest(code, std::move(completion));
            return true;
        }
    }

    admission_error.error = "Lua engine is not initialized.";
    if (completion) {
        completion(std::move(admission_error));
    }
    return false;
