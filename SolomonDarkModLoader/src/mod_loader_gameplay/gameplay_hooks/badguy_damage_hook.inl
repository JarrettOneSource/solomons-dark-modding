...
#include "shared_stock_xp_hook.inl"
...
std::uint8_t __fastcall HookBadguyDamage(
    ...
    if (have_source &&
        ...
        Log("[bots] authority synthetic Fireball native damage authorized. monotonic_ms=" +
    ...
    const auto shared_stock_xp_capture =
        CaptureSharedStockXpBeforeDamage(
            actor_address,
            context_source);
    ...
    const auto call_original = [&]() {
        ...
        ArmSharedStockXpAfterDamage(
            actor_address,
            result,
            shared_stock_xp_capture);
        return result;
    };
