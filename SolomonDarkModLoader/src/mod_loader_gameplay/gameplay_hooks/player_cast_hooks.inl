...
bool TryCaptureLocalSecondaryCast(
    ...
    *capture = LocalSecondaryCastCapture{};
    ...
    constexpr std::uint64_t kRecentBeltSlotEdgeWindowMs = 500;
    std::int32_t recent_belt_slot = -1;
    const auto now_ms = static_cast<std::uint64_t>(GetTickCount64());
    if (TryGetRecentGameplayBeltSlotEdge(
            now_ms,
            kRecentBeltSlotEdgeWindowMs,
            &recent_belt_slot) &&
        capture->secondary_entry_indices[
            static_cast<std::size_t>(recent_belt_slot)] == skill_entry_index) {
        capture->belt_slot = recent_belt_slot;
    } else {
        const auto found = std::find(
            capture->secondary_entry_indices.begin(),
            capture->secondary_entry_indices.end(),
            skill_entry_index);
        if (found != capture->secondary_entry_indices.end()) {
            capture->belt_slot = static_cast<std::int32_t>(
                std::distance(
                    capture->secondary_entry_indices.begin(),
                    found));
        }
...
#include "registered_spell_input.inl"
...
#include "player_secondary_spell_cast_hook.inl"
...
void __fastcall HookPurePrimaryAttackDispatch(void* self, void* /*unused_edx*/) {
    ...
    bool authorize_authority_synthetic_damage = false;
    ...
    std::vector<uintptr_t> projectile_addresses_before;
    {
        ...
        if (binding != nullptr &&
            ...
            authorize_authority_synthetic_damage =
    ...
    if (authorize_authority_synthetic_damage &&
        expected_projectile_type == kFireballDamageSourceNativeTypeId) {
        RememberAuthoritySyntheticDamageSource(
            emitted_projectile_actor,
            binding->bot_id);
        Log(
            "[bots] authority synthetic Fireball damage source registered. "
            "participant_id=" +
            std::to_string(binding->bot_id) +
