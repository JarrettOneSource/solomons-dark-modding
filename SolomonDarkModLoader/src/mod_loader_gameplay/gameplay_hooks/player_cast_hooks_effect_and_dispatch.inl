void RememberRecentNativeSpellEffectActor(
    uintptr_t actor_address,
    std::uint32_t native_type_id) {
    if (actor_address == 0 || native_type_id == 0) {
        return;
    }
    ...
    if (it != g_recent_native_spell_effect_actors.end()) {
        it->native_type_id = native_type_id;
        it->created_ms = now_ms;
        return;
    }

    SDModNativeSpellEffectActorState actor{};
    actor.actor_address = actor_address;
    actor.native_type_id = native_type_id;
    actor.created_ms = now_ms;
    g_recent_native_spell_effect_actors.push_back(actor);
}

uintptr_t __fastcall HookFireEmberCtor(void* self, void* /*unused_edx*/) {
    const auto original = GetX86HookTrampoline<NativeSpellEffectCtorFn>(
        g_gameplay_keyboard_injection.fire_ember_ctor_hook);
    if (original == nullptr) {
        return 0;
    }

    const auto actor_address = original(self);
    RememberRecentNativeSpellEffectActor(actor_address, 0x07D6);
...
void __fastcall HookPlayerActorPurePrimaryGate(void* self, void* /*unused_edx*/) {
    ...
    // A per-cast primary remains tracked until its first native projectile
    // disappears so impact/damage can finish. That observation window must not
    // keep invoking the emission gate: doing so turns one replicated press into
    // a stream of extra Fire/Ether/Water projectiles.
    if (HasNativeRemotePerCastProjectileEmission(actor_address, nullptr)) {
        return;
    }
