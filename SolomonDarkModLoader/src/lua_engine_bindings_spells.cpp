...
const LuaSpellDefinition* ResolveSelectableSpell(
    lua_State* state,
    int argument_index) {
    if (lua_type(state, argument_index) == LUA_TSTRING) {
        auto* mod = GetLoadedLuaMod(state);
        if (mod == nullptr) {
            luaL_error(state, "sd.spells.select requires an owning Lua mod");
            return nullptr;
        }
        std::size_t key_length = 0;
        const auto* key =
            lua_tolstring(state, argument_index, &key_length);
        return FindOwnedSpellDefinitionByKey(
            *mod,
    ...
    if (lua_isinteger(state, argument_index)) {
        ...
        if (raw_id <= 0) {
            luaL_error(
                state,
                "sd.spells.select id must be a positive integer");
            return nullptr;
        }
        return FindSpellDefinitionById(static_cast<std::uint64_t>(raw_id));
    }
    luaL_error(
        state,
        "sd.spells.select expects a content key or content id");
    return nullptr;
}

void PushSelectedSpellDefinition(
    lua_State* state,
    const LuaSpellDefinition& definition,
    ...
    if (definition.slot == LuaSpellSlot::Secondary) {
        lua_pushinteger(state, secondary_slot + 1);
        lua_setfield(state, -2, "belt_slot");
    }
}
...
#include "lua_engine_bindings_spells_runtime.inl"
