...
void ApplyTransform(ParticipantInfo* participant, float position_x, float position_y, float heading) {
...
bool UpdateBot(const BotUpdateRequest& request) {
    ...
    UpdateRuntimeState([&](RuntimeState& state) {
        ...
        if (request.has_transform) {
            ApplyTransform(participant, request.position_x, request.position_y, request.heading);
        }
    ...
    if (updated && request.has_transform) {
