...
#include "run_loading_visibility.inl"
...
void ServiceRunLoadingBarrier(std::uint64_t now_ms) {
    ...
    const bool world_checkpoint_ready =
        ...
        (runtime_state.world_snapshot.valid &&
         runtime_state.world_snapshot.run_nonce == run_nonce);
    const bool wave_checkpoint_ready =
        g_local_transport.is_host ||
        runtime_state.host_wave_checkpoint_run_nonce ==
            run_nonce;
    if (!g_local_transport.is_host &&
        world_checkpoint_ready) {
        AdvanceLoadingScreen(
            LoadingScreenStage::ReceivingWorldCheckpoint);
    }
    if (!g_local_transport.is_host &&
        world_checkpoint_ready &&
        wave_checkpoint_ready) {
        AdvanceLoadingScreen(
