...
$source = Join-Path $root "tools/win32_real_input.cpp"
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Join-Path $root "runtime/tools/win32_real_input.exe"
}
...
& cl.exe /nologo /O2 /EHsc /W4 /WX /DUNICODE /D_UNICODE "/Fo:$objectPath" "/Fe:$OutputPath" $source user32.lib
if ($LASTEXITCODE -ne 0) {
    throw "Win32 real-input helper build failed with exit code $LASTEXITCODE."
}
