...
MUTATIONS: tuple[Mutation, ...] = (
    ...
    TextMutation(
        "replay.embedded-standalone-agreement",
        RENDERER,
        contracts.LAYOUT_REPLAY,
        "standalone.layout",
        "standalone.mutantLayout",
        "T2 replay no longer compares standalone and embedded layout copies",
    ),
    TextMutation(
    ...
    TextMutation(
        ...
        "screens=${catalog.layouts.size}/unknown",
        "T2 replay no longer reports the complete 28-screen census",
    ),
    TextMutation(
    ...
    TextMutation(
        "traversal.edge-floor",
        TRAVERSAL,
        contracts.CONTROLLER_TRAVERSAL,
        "assert.equal(edges.size, 39",
        "assert.equal(edges.size, 38",
        "controller traversal no longer ingests the 39-edge live G11 graph",
    ),
    TextMutation(
        ...
        "    [...visited].sort(),",
        "    [...visited],",
        "controller traversal no longer proves exact live-edge set equality",
    ),
    TextMutation(
        "traversal.human-log",
        TRAVERSAL,
    ...
    TextMutation(
        "traversal.map-picker-semantics",
        TRAVERSAL,
        contracts.CONTROLLER_TRAVERSAL,
    ...
    TextMutation(
        "traversal.map-boundary",
        TRAVERSAL,
        contracts.SHELL_CONTROLLER,
        'this.#enterOutOfScope("Gameplay and Boneyard startup are outside P0;',
        'this.#enterHub(); // Gameplay and Boneyard startup are outside P0;',
        "map Start no longer terminates visibly at the explicit P0 gameplay boundary",
    ),
    TextMutation(
        "traversal.multiplayer-boundary",
        TRAVERSAL,
        contracts.SHELL_CONTROLLER,
        "Multiplayer, rooms, hub gameplay, and Boneyard gameplay are outside P0.",
        "Gameplay follows silently.",
    ...
    TextMutation(
        ...
        "bypassMenuVisualGate(menuVisualGateJson, catalog)",
        "live capture no longer applies the enumerated visual gate",
    ),
    TextMutation(
        "visual.capture-records-artifacts",
        VISUAL,
        contracts.CAPTURE,
