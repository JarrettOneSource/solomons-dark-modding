...
def test_cursor_placed_secondaries_replay_owner_world_position() -> str:
    ...
    assert "constexpr std::uint16_t kProtocolVersion = 92;" in protocol
...
def test_network_clients_reject_stock_incoming_damage_authority() -> str:
    ...
    assert "const auto result = original(self);" in hook
    assert "SuppressNativeRemoteParticipantTerminalDispatch(" in hook
    assert "return result;" in hook
