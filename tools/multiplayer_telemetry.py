...
emit_num("player.render_drive_effect_timer", player and player.render_drive_effect_timer or 0)
emit_num("player.render_drive_effect_progress", player and player.render_drive_effect_progress or 0)
...
for index, peer in ipairs(peers) do
  ...
  emit_num(prefix .. "render_drive_effect_timer", peer.render_drive_effect_timer or 0)
  emit_num(prefix .. "render_drive_effect_progress", peer.render_drive_effect_progress or 0)
  ...
  if actor ~= 0 and sd.debug and sd.debug.layout_offset then
    ...
    emit_num(prefix .. "actor_effect_timer", sd.debug.read_float(actor + otimer) or 0)
    emit_num(prefix .. "actor_effect_progress", sd.debug.read_float(actor + oprogress) or 0)
