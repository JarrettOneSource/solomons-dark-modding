#!/usr/bin/env python3
"""Mutation-test the registered v2.20 static contract itself."""
...
CASE = "static_contract_rejects_case_mutated_target"
GREEN_OUTPUT = (
    "Settlement v2.20 corrects exactly one Dark Cloud login title field, "
    "pins both settled instances and endpoints, and preserves production "
    "stop-first behavior"
)
TRIP_MESSAGE = (
    "Settlement v2.20 no longer authorizes exactly the one-field Dark Cloud "
    "login title correction"
)
...
def require_green(completed: subprocess.CompletedProcess[str], phase: str) -> None:
    if (
        ...
        raise RuntimeError(
            f"v2.20 static contract {phase} was not green: "
...
def main() -> int:
    ...
    if not isinstance(parsed, dict) or parsed.get("settled_value") != "Dark Cloud Browser":
        raise RuntimeError("v2.20 static mutation did not reach the exact target field")
    ...
    mutated = dict(parsed)
    mutated["settled_value"] = "DARK CLOUD BROWSER"
    try:
        atomic_bytes(
            contract_path,
    ...
    if (
        ...
        raise RuntimeError(
            "v2.20 case-mutated contract did not trip its exact registered claim"
        )
    ...
    atomic_json(
        trip_path,
        {
            "case": CASE,
            "phase": "trip",
            "edit": "change the committed contract settled_value case only",
    ...
    if contract_path.read_bytes() != original:
        raise RuntimeError("v2.20 static mutation did not restore the exact contract bytes")
    ...
    table = {
        "schema": "solomon-dark-native-menu-v220-static-contract-mutation-table-v1",
        "row_count": 1,
        "all_passed": True,
        "rows": [
            {
    ...
    atomic_json(output / "v220-static-contract-mutation-table.json", table)
