...
| Skills/disciplines | `docs/reverse-engineering/native-progression-and-skills.md`, live `tests/fixtures/webgame/progression-goldens.json`, registered `tests/re/static_re_native_progression_contracts.py`, plus the inherited concentration/discipline and picker artifacts | HIGH — G6 closed; exact per-actor XP/level/offers, all 82 skill rows, and discipline semantics are pinned |
...
| G6 progression & per-skill effects | progre | XP/level curves, level-up offer pools, exact effect semantics per skill (incl. the Firewalker hoard `+0x740` model from botmana), discipline modifiers. | **CLOSED 2026-08-06** — [per-actor progression and all-skill effect contract](reverse-engineering/native-progression-and-skills.md), [live XP/offer/effect goldens](../tests/fixtures/webgame/progression-goldens.json), and [five registered static RE contracts](../tests/re/static_re_native_progression_contracts.py). |
...
| G10 saves/accounts | savere | Save format, account linkage (launcher Settings owns saves today). Needed only by launch. | **CLOSED 2026-08-06** — [native save format, lifecycle, launcher layer, and P6 account seam](reverse-engineering/native-save-format.md), [three isolated live round-trip goldens](../tests/fixtures/webgame/save-format-goldens.json), and [registered static contracts](../tests/re/static_re_native_save_format_contracts.py). The current selected-slot stage-routing defect is recorded explicitly; no website feature was built. |
