...
| G4 animation & presentation state machines | animre | Wizard + enemy sprite state transitions (idle/walk/cast/hit/death), frame timings, attachment points (staff orb type 7004), lighting/shadow model, camera constants (`native-camera-control.md`). Consumes G12's composition spec. **The G2 emitter dependency is no longer blocking** (ATC 2026-08-05): the directional cast-glyph point list is recovered — records `#3244..#3483` of `images/Clothes.bundle` as 10 banks × 24 facings, point index 1 of the stride-`0xC4` record, indexed `facing + 24*K`. See *The cast glyph emitter* in `native-projectile-and-spell-mechanics.md`. What this campaign still owes on that thread: name the bank source `actor->+0x238` in animation terms (goldens show `K=0` for the first cast tick then `K=7`), identify what the other two point runs (`#460..#579` = 5 × 24, `#796..#867` = 3 × 24) attach to, and sweep captures across several headings so the facing formula is observed rather than derived at 23 of 24 facings. Goldens: frame-by-frame animation captures with timestamps, plus emitter points across all 24 facings. | QUEUED |
...
| G7 stock loot/reward selector | lootre | Drop-rate tables, gold amounts, potion selection, magnet physics constants; builds on `native-items-equipment-and-loot.md` + potiondrop findings. **Inherits the open G1 seeding-lifecycle residual** — drops run off an actor-seeded private stream (`0x0047C070`), so record that stream's seed source directly instead of relying on the shared-global determinism claim. | QUEUED |
