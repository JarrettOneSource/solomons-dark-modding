...
| Gap | Campaign | Scope | Status |
| --- | --- | --- | --- |
| G1 sim core: movement integrator, tick graph, RNG | **physre** | Input→intent→velocity→position pipeline with exact constants (base speeds, diagonal handling, knockback, wall response via `movement_collision_test_circle_placement`); the full tick graph (which systems at which cadence, in what order — reconcile 67 ms motion + 250 ms mana + render frames against `game-timing-scale.md`); native RNG algorithm, state, seeding, per-system streams, gameplay call-site census. | **CLOSED 2026-08-04** — [movement, tick, and RNG mechanics](reverse-engineering/native-movement-and-tick.md), [live movement goldens](../tests/fixtures/webgame/movement-goldens.json), [live RNG goldens](../tests/fixtures/webgame/rng-goldens.json), and [static RE contracts](../tests/re/static_re_native_sim_core_contracts.py). **Two residuals (ATC re-audit 2026-08-05, see that doc's Not Yet Reversed):** the generator itself is portable and re-derived bit-exactly from prose (352 outputs, 55/55 state words, 4 seed/range pairs), but (a) the **seeding lifecycle is unverified** — the one recorded snapshot inverts uniquely to `NativeRng_Seed(5683095)` + 2 draws, which is neither the published seed `19088743` nor a post-generation state, so nothing shows `sd.rng.set_seed` controlling world generation; (b) the **float primitive has no goldens at all**. Both must be closed before any rolled system is ported. |
...
| G4 animation & presentation state machines | animre | Wizard + enemy sprite state transitions (idle/walk/cast/hit/death), frame timings, attachment points (staff orb type 7004), lighting/shadow model, camera constants (`native-camera-control.md`). Consumes G12's composition spec. **Inherits a blocking G2 dependency:** the cast glyph emitter point is animation asset data, not a code constant, so this campaign must recover the directional cast-glyph point list — all 24 facings per bank, point index 1 of the stride-`0xC4` record — or P3 cannot place a projectile spawn for any facing but `287.59668`. See the Not Yet Reversed section of `native-projectile-and-spell-mechanics.md`. Goldens: frame-by-frame animation captures with timestamps, plus emitter points across all 24 facings. | QUEUED |
