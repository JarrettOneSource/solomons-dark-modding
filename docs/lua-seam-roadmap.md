...
**Implemented 2026-07-22.** Protocol 89 provides a bounded, deterministic Lua value
...
participant over protocol 89, and lets that owner resolve its peer-local recipe UID just
...
Protocol 89 carries the content ID and effective constructor values through world
...
identities to bounded immutable config and owner-state callbacks. Protocol 89 routes host
...
includes planned composition, and protocol 89 carries a bounded validated
...
- **`sd.time.set_scale()`** — slow-mo, pause, frame-step. *MP:* shared-simulation variable
  ...
  **Implemented 2026-07-23.** `sd.time` provides
  ...
  mutate time; protocol 89 repeats scale/revision state and sends prompt authenticated
