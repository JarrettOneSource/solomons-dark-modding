...
**Implemented 2026-07-22.** Protocol 84 provides a bounded, deterministic Lua value
...
participant over protocol 84, and lets that owner resolve its peer-local recipe UID just
...
Protocol 84 carries stable content IDs through drops, pickup results, inventory,
...
Protocol 84 carries the content ID and effective constructor values through world
...
identities to bounded immutable config and owner-state callbacks. Protocol 84 routes host
...
cross-class call. Clients run no mod AI and receive the resulting protocol-82 world
...
reliable protocol-82 authority requests authenticated by endpoint, participant
...
binary-safe payloads through a bounded protocol-82 fragment envelope. Clients send only
...
includes planned composition, and protocol 84 carries a bounded validated
...
- **`sd.time.set_scale()`** — slow-mo, pause, frame-step. *MP:* shared-simulation variable
  ...
  **Implemented 2026-07-23.** `sd.time` provides
  ...
  mutate time; protocol 84 repeats scale/revision state and sends prompt authenticated
