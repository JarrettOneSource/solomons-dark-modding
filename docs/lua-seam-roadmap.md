...
**Implemented 2026-07-22.** Protocol 86 provides a bounded, deterministic Lua value
...
participant over protocol 86, and lets that owner resolve its peer-local recipe UID just
...
Protocol 86 carries stable content IDs through drops, pickup results, inventory,
...
Protocol 86 carries the content ID and effective constructor values through world
...
identities to bounded immutable config and owner-state callbacks. Protocol 86 routes host
...
includes planned composition, and protocol 86 carries a bounded validated
...
- **`sd.time.set_scale()`** — slow-mo, pause, frame-step. *MP:* shared-simulation variable
  ...
  **Implemented 2026-07-23.** `sd.time` provides
  ...
  mutate time; protocol 86 repeats scale/revision state and sends prompt authenticated
