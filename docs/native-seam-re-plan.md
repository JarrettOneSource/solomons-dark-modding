...
rate, no negative native delta hits on the gameplay player actor, no gameplay
player MP decrease, and a real MP decrease on the bot's live progression state. The native
...
The 10%/80% bot mana reserve covers active held casts before stock
`PlayerActorTick` can keep native primary input held; reserved idle bots recover
their own progression pool through native `0x0052B150` positive deltas until
they exit reserve, rather than direct MP writes or a shared player pool.
...
- Some stock cast paths still resolve mana through gameplay slot 0 even when
  the ticking actor is a bot. Ghidra later showed `0x0052B150` also has a
  zero-MP callback that hardcodes `gameplay+0x1654` slot 0. Native bot casts
  therefore scope the progression-slot owner context to narrow stock cast calls
  and the native `PlayerActorApplyManaDelta` function itself, not to the whole
  active bot `PlayerActorTick`. Before stock ticks and mana deltas, the loader
  keeps each bot actor's direct progression runtime cache (`actor+0x200`) bound
  to the actor's own smart-pointer handle. Slot 0 is restored immediately after
  each scoped native call, so spell construction, charge/release, damage,
  cleanup, held-cast mana mutation, and the zero-MP callback all stay on the
  correct per-player resource pool without returning to a synthetic local-player
  mana window.
- The earlier guessed bot-owned `Skills_Wizard::Tick` vfunc seam is retired.
  Live write-watch evidence showed it was not a verified stat-pool tick path:
  the candidate call could enter unrelated parser/data routines and write
  through the local player's progression memory. Bot stat mutation therefore
  stays on recovered stock cast handlers and scoped progression ownership until
  a real native per-actor stat tick seam is proven.
...
- `tests/re/run_live_bot_native_mana_spend_probe.py` validates the bot path in
  ...
  local-player actor pointer and MP unchanged.
...
- Earth boulder release/damage was deepened with Ghidra and live evidence:
  ...
  - `0x005E5450`: native finalizer computes released damage from the live
    ...
    The dispatcher conditionally applies the native `0x007DE808` half-scale
    to the damage/base-damage fields only when its below-maximum release
    predicates pass; the finalizer then clamps with the native floor at
    `0x007DE8F0` and cap scale at `0x00784740`.
  ...
  - `0x004621B0`: the object `+0x58` vfunc reads the world's active query
    ...
    primary-stat normalization constant at `0x007A03F0` is excluded from
    Boulder contact damage; its sole retail reference is in the primary stat
    output builder, not the dispatcher, finalizer, or contact chain. The
    active release policy compares live target HP against the finalizer curve
    derived from the live `Boulder + 0x1F8` base and charge; any conditional
    half multiplier is already embodied in that native field. It uses the
    stock max-size release path when lethal damage plus native victim-scan
    eligibility is not yet proven. While the boulder is still charging, the
    projection follows the bot's live target through the same
...
| Manual mana spend | `bot_casting/resource_state.inl`, `pending_cast_processing.inl`, `native_cast_gate_patches.inl`, `core/standalone_progression_slot_context.inl`, `gameplay_hooks/player_mana_hooks.inl` | done: player MP writer watch, `0x0052B150` native delta function, `0x0052B171` local-actor gate branch, stock spell-handler callers in `runtime/ghidra_mana_delta_callers.txt`, live bot-actor negative delta traces, Ghidra evidence that the native zero-MP callback hardcodes slot 0, and live guards for no gameplay-player negative delta or MP loss | done: native local-actor gate is layout-backed, validated as a live `jnz rel32` branch, captured for restore, and patched once; stock spell handlers own bot MP mutation, while loader code only performs live MP admission/depletion observation, scopes slot-0 progression ownership to the current bot during narrow stock cast calls and `PlayerActorApplyManaDelta`, keeps each bot actor's direct progression runtime cache bound to its own pool, repairs slot 0 back to the local player's progression after scoped native calls, and never swaps `gameplay+0x1358` or manually spends MP |
| Earth boulder damage | `resource_state.inl`, `boulder_damage_projection.inl`, `badguy_damage_hook.inl` | done: complete actor-stat, dispatcher, conditional-release, finalizer, contact, and claim-path trace; active Boulder release-base field `0x1F8`; primary-stat normalization constant `0x007A03F0` proven absent from stock contact; finalizer floor/cap globals `0x007DE8F0`/`0x00784740`; native victim-scan predicate; and exact two-owner issue-#52 matrix | done: native code exclusively owns Boulder `+0x1F4/+0x1F8`; projection reads the live base and mirrors the native charge-square/floor/cap curve without a universal half multiplier; packet-driven remote contacts cannot mutate replicated enemy HP while the local owner applies stock contact directly or through the client claim path; held charging still follows the live target, freezes it at release, preserves the stock input falling edge, and pins only charge/release-charge plus the recovered growth-stop field |
| Synthetic source profiles | removed hardcoded profile buffer and element-color table | done for active code: native consumer `0x005E3080`, clone path `0x0061AA00`, source actor ctor `0x005E9A90`, rejected create-screen discipline trim mapping at `0x005D0290`, native `Skills_Wizard` element color seam `0x00660760`, and native robe mix helper `0x0040FC60` are documented; live probes verify finalized actors keep `actor+0x178 == 0`; `runtime/ghidra_color_vector_helpers.txt` and `runtime/ghidra_color_mix_scalar_scan.txt` keep the native color-helper evidence current | done: active runtime stages a transient native-derived source profile from `0x00660760`, converts the descriptor-facing native color through the recovered `0x0040FC60` source-profile preimage, reads trim from the native source actor default, lets `0x005E3080` build the descriptor, clears the staging pointer before clone publication, leaves standalone clone actor-local render/animation counters native-owned, publishes descriptors through stock clone/helper lanes, writes only safe source-built selector bytes onto target actors, and has no hardcoded element color table |
...
| Standalone collision | scoped participant collision resolver | done for active code: `0x0061AA00`/`0x0063F6D0`/`0x005217B0` prove native clone registration and grid-cell binding; `0x00521B80`/`0x00522500`/`0x00522C00`/`0x00522B20` prove the stock overlap-response context; `runtime/ghidra_standalone_collision_ownership_xrefs.txt` and `runtime/ghidra_standalone_collision_field_writes.txt` confirm `0x00622D90`/`0x005217B0` are position-rebind/cell-repair seams and that `0x00525800` owns the direct overlap-response calls, while `tests/re/run_live_standalone_collision_probe.py` and `runtime/live_standalone_collision_probe.json` preserve the historical live evidence for the removed bridge | done: active tick code runs `ResolveWizardParticipantActorCollisions` as a scoped participant collision resolver: the local player stays solid, bot/bot and player/bot overlaps move only bot-owned actors, moved bots are repaired through `WorldCellGrid_RebindActor`, and the old standalone-only push bridge remains removed until a native dynamic overlap-response lifecycle is recovered |
...
| Slot-0 cast shim | removed `local_player_cast_shim.inl`; added `native_cast_gate_patches.inl` and `core/standalone_progression_slot_context.inl` | done: `runtime/ghidra_cast_slot0_dispatch_xrefs.txt`, `runtime/ghidra_cast_slot0_gate_offset_accesses.txt`, and `runtime/ghidra_pure_primary_emit_handlers_disasm_latest.txt` identify the exact slot gates; `0x0052F3B9`, pure-primary Ether/Fire gates `0x0053D1B3`/`0x0053D9D2`/`0x0053E4E8`, `0x00544C92`, `0x00545393`, `0x00545C2C`, and MagicMissile impact damage gate `0x005F1F39` are now layout-backed native cast gate patches validated from live instruction shape, so gameplay-slot bots keep their real `actor+0x5C` and live progression slot while stock handlers still allocate their projectile objects and their hits still reach native damage; the second Ether gate at `0x0053D9D2` protects the native `0x7D3` projectile allocation loop, while `0x005F1F39` bypasses the hit handler's projectile-group `+0x5C` damage branch for bot-owned `0x7D3` projectiles; standalone slot-0 resource lookups are scoped to the bot's progression wrapper only during stock cast calls | done: no local-player actor/window shim remains in active code; failed branch validation aborts input injection instead of silently falling back, and standalone slot owner context restores the original slot wrapper after each stock call |
