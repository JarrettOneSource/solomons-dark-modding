...
| Lua bot constants | `mods/lua_bots/scripts/lib/lua_bots/config.lua`, `mods/lua_bots/scripts/lib/lua_bots/combat.lua` | partial/done: C++ scene actor state publishes native-backed `tracked_enemy`; `docs/lua-bot-constants-re.md` classifies water range and entry indices as native-derived, and follow/travel/attack windows as policy | semantic enemy targeting done; keep documented policy tuning until native behavior or hub/entrance APIs are recovered |
