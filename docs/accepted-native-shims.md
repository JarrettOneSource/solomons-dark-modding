...
| Active spell object lookup and Boulder release observation | Charged Boulder damage is owned by the live spell object and native progression state. The loader observes the active object, projects lethal release from its native base, and controls only charge/growth timing; it never writes the native damage fields. | Re-run target lethality after target swaps using the same charged Boulder state. Packet-driven remote contacts stay presentation-only; client-owned stock contact uses the ordinary enemy-damage claim path. |
