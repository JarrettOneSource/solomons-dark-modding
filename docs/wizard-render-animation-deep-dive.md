...
- copies the signed `IDLE BEHAVIOR` byte at `source +0x56` to actor
  `+0x1C0`; `GameNPC` mode 1 continuously faces the local Player
- copies `TALK SPEED` at `source +0x74` to actor `+0x194`; the field is
  serialized and value 1 enables the corresponding dialog-object mode
- packs cloth/trim float4 colors from `source +0xB4` and `source +0xC4` into the actor-side descriptor block at `+0x244..+0x263`
  ...
  - trim stays on the native source actor default trim color; the stock
    materialization chain contains no later trim owner or override
...
The item-render path is exact:

- `Staff_RenderAttachment (0x00578D20)` indexes Clothes records 5..10 by the
  staff selector;
- an optional glow-color argument adds Clothes records 11..12 and a generated
  four-vertex colored/flickering quad;
- the active pose selects from Clothes records 3244..3483 and 3484..3723;
- helpers `0x005795E0` and `0x00579680` return the frame-specific attachment
  points used by the staff and wand paths;
- the glowing staff orb and other weapon-specific visuals therefore come from
  the staff/wand item's own rendering behavior;
- `actor + 0x264` is the staging pointer that exposes that item to the
  source-side render path;
...
| `+0x1C0` | `idle_behavior` | signed mirror of source `IDLE BEHAVIOR`; GameNPC value 1 faces the local Player |
...
| `+0x56` | `idle_behavior` | signed `IDLE BEHAVIOR`; mirrored into actor `+0x1C0` |
| `+0x74` | `talk_speed` | `TALK SPEED`; mirrored into actor `+0x194` |
...
## 3.3 Bounded causes of the "laying flat" failure mode

The executable does not attach human-readable authoring names to every numeric
selection ID. The numeric mapping and the failure gates below are exact, so the
failure can still be bounded without inventing labels.
...
The bounded failure set is therefore:
...
## 9. Native visual-contract closure

The two former open questions are closed:

- the staff renderer itself owns the visible glow composition: selector
  records 5..10, optional glow records 11..12, a generated colored/flickering
  quad, pose banks 3244..3723, and frame-specific attachment-point tables;
- stock clone does not contain a later step that republishes the source's
  packed `+0x244..+0x263` descriptor onto the clone actor. The clone factory
  intentionally copies that payload into robe/hat helper items, transfers the
  weapon item into equip sink `+0x30`, and leaves clone-local counters and
  capacities native-owned. Searching for a clone-side descriptor repopulation
  step was based on the wrong ownership premise.

The resulting ownership model is complete: the orb comes from the attachment
item path, not the actor body descriptor block or robe/hat helpers; source
actors carry the packed body descriptor; and standalone clones publish the
source appearance through equip-linked helpers without overwriting their live
actor-local animation state.
