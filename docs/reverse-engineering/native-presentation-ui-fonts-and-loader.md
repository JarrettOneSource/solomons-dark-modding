...
## 2026-08-23 reusable stock UI building-block seam

### Boundary and evidence refresh

The reusable system is the stock presentation vocabulary below the individual
screen state machines: exact atlas records, bitmap-font wrappers, composition
geometry, semantic hit rectangles, painter order, and renderer teardown. A
Title, Dark Cloud, Inventory, SkillScreen, editor, HUD, or other screen still
owns its own state, actions, timing, input policy, and transition lifecycle.
The shared `UI` singleton never becomes a screen controller merely because
several screens select its records.

This boundary was refreshed against retail `SolomonDark.exe` 0.72.5, size
`4,723,200`, SHA-256
`03a834566ce70fd8088f4cf9ee6693157130d8aec28c092cb814d6221231f1e3`,
preferred image base `0x00400000`. The canonical read-only `SolomonDark`
Ghidra project was queried through replica slot 1 with the repository wrapper:

- `decompile_targets.py 0x004F3590 0x0043BCD0 0x005C65A0 0x005C6A50
  0x005A0960 0x00594FC0` re-confirmed the 113-record `UI` builder,
  `ExactText_Render`, the labeled/unlabeled shared control renderers, the
  shared panel path, and the Dark Cloud browser consumer;
- `trace_singleton_register_offsets.py 0x008199E4 30` found 331 compiled
  singleton loads and re-confirmed register-derived record selection across
  Inventory, HUD, Hall, Dark Cloud, skill, panel, and control owners. Material
  examples include records 81/82 at `0x005C876B..0x005C87FA`, records 47/48 at
  `0x005D7950..0x005D7A6D`, the mouse-button vector at `0x005D5087`, and the
  shared labeled/unlabeled control families at `0x005C60F0`, `0x005C65A0`,
  and `0x005C6A50`.
- focused decompilation of `UiLabeledControl_Render 0x005C60F0`, its alternate
  `0x005C65A0`, and `UiUnlabeledControl_Render 0x005C6A50`, followed by
  `dump_floats_at.py 0x007DE870 0x007DE978`, closes the disabled branch. A
  disabled control draws its selected record and label at alpha `0.5`, then
  draws its control rectangle with RGBA `(0.5,0.5,0.5,0.25)` before restoring
  white/opaque state.

The exhaustive source membership is now materialized in
[`native-ui-kit-catalog.json`](native-ui-kit-catalog.json), generated by
`tools/build_native_ui_kit_catalog.py`. The generator joins
`native-content-inventory.json` to `native-atlas-consumers.json`; it does not
infer geometry from contact-sheet pixels. Its record rows are the normative
one-row-per-asset inventory and carry one allowed disposition each.

### Complete source membership

| Atlas | Builder | Records | Building-block disposition |
| --- | ---: | ---: | --- |
| `Bonedit` | `0x004E41C0` | 84 | `exact-ported` |
| `ControlPanel` | `0x004E7EF0` | 116 | `exact-ported` |
| `Controls` | `0x004E84E0` | 4 | `exact-ported` |
| `Create` | `0x004E8680` | 24 | `exact-ported` |
| `Fonts` | `0x004EA3D0` | 627 | `exact-ported` |
| `GameOver` | `0x004EA650` | 3 | `exact-ported` |
| `Inventory` | `0x004EB0F0` | 84 | `exact-ported` |
| `LevelPicker` | `0x004EBA90` | 8 | `exact-ported` |
| `Loader` | `0x004EC1F0` | 5 | `exact-ported` |
| `Skills` | `0x004ED280` | 166 | `exact-ported` |
| `Title` | `0x004F3210` | 25 | `exact-ported` |
| `UI` | `0x004F3590` | 113 | `exact-ported` |

That is 12 atlases and 1,259 exact records. Porting a record means preserving
its packed frame, logical size, trim origin, atlas identity, and source hashes;
it does not mean making a dormant native record reachable from an existing
stock screen. The catalog keeps the proved dormant classifications from this
report and the Skills closure, but exposes those records deliberately so a new
web-authored surface can reuse stock art without modifying the stock behavior
of an old surface.

The font membership is also complete: Fonts groups 0 through 8 plus the
ControlPanel wrapper, 718 glyph records total. Every wrapper row preserves its
three-float header, finite glyph ids, glyph metrics, space advance, and full
kerning table. Unsupported characters remain absent; no system-font fallback
is part of the stock interface.

### Reusable primitive membership

| Primitive | Recovered stock contract | Disposition |
| --- | --- | --- |
| atlas sprite | exact record frame, logical size, trim origin, transforms, alpha, tint | `exact-ported` |
| bitmap text | wrapper-selected glyphs, kerning, measurement, alignment, wrap, tint | `exact-ported` |
| tiled/clipped fill | exact record repeated or clipped within caller geometry | `exact-ported` |
| mirrored frame / nine-slice | authored corner, final-5-percent edge sampling where required, deterministic mirror/order | `exact-ported` |
| button | `UI.101` idle, `UI.102` pressed, mirrored `UI.54` ends, Fonts group 3 label | `exact-ported` |
| tab strip | `UI.13` bracket pair; selected label rises 8 px and brackets grow from 51 to 65 px without X drift | `exact-ported` |
| message box | `UI.107..110` outer corners, `UI.10/79` edges, `UI.49` fill, `UI.17` inner frame, `UI.18` header, `UI.8` arrows | `exact-ported` |
| SimpleMenu | stock frame and rows plus owner-supplied reveal/close timing and semantic hit bounds | `exact-ported` |

The canonical 30 settled layouts, `dark_cloud_settings_credentials` overlay,
and `beta_notice_first_boot` composite are each enumerated in the catalog as
`out-of-system`: they consume the vocabulary but retain their own screen
behavior. This is deliberate locality, not missing membership. The component
layer must not absorb save selection, settings persistence, tab data loading,
...
are resident, but the adapter owns and destroys those derived textures. The
scene owner still acquires and releases the source page set and destroys its
containers. Text layout is pure and shared by the WebGL and semantic-DOM
adapters; the two presentation adapters do not get independent kerning or
fallback rules. Semantic controls consume the same returned hit rectangles as
the visible plan, so CSS coordinates cannot drift from WebGL geometry.

UI selection is presentation-local and has no host authority, simulation tick,
save, protocol, replay, or RNG state. Individual screen actions may still be
authoritative through their existing owners; creating a visual stock button
does not create an action or grant authority. There is no browser-platform
blocker: every recovered operation is representable by atlas subtextures,
mirroring, tiling/clipping, bitmap glyph sprites, and semantic HTML hit targets.
