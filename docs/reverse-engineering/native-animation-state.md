...
| `Imp 0x3EC` | `airborne`, `fly`, `contact/cooldown`, `hit_overlay`, `death_handoff/split` | BadGuys `285..342`; `facing + 12*constructorVariant(+0x220)`; 12 facings; renderer `0x00492E10` |
...
| `Wraith 0x3EF` | `idle/approach/orbit/retreat`, `attack/cooldown`, `hit_overlay`, `death_handoff` | one facing-only BadGuys strip `2070..2087`; 18 facings; the body renderer does not consume an action pose or fade-alpha lane; renderer `0x00496220` |
...
| `Coffin 0x3F5` | `closed`, `opening`, `transition_delay`, `open`, `hit_overlay`, `death_handoff` | no facing; direct lid/body phase selects BadGuys `175..187` and `383..392`; retained constructor X scale `+/-1` and signed `Float(15)` rotation transform both body rendering and launch segments; renderer `0x0049AC90` |
| `Maggot 0x7FD` | `ballistic_emerge`, `crawl`, `bite`, `death_handoff` | ballistic lane is `orientation(0..9) + 10*trunc(phase)` over BadGuys `2013..2062`; constructor phase is `Float(5)` and tick adds float32 `0.25` with wrap at `>=5`; grounded lane is `facing + 18*pose(+0x238)` over BadGuys `202..237`; 10 airborne orientations, 18 grounded facings; renderer `0x0049C190` |
...
| Wraith | `idle/approach/orbit/retreat <-> attack/cooldown`; any alive -> death handoff | action state changes movement, attacks, and owned effects, not the facing-only body record or body opacity |
...
| Zombie locomotion/body | 18-way articulated selectors over arms `2095..2202`, body `2203..2274`, body-3 overlay `2275..2292`, head `2293..2364`, and base `2365..2508`; the renderer transforms all three authored body points before placing head/arms | constructor `0x004740C0` seeds `+0x210/+0x214` in `[0,360)`, signed head angle `+0x218` in `[-65,65)`, arm angles `+0x228/+0x22C` in `[0,20)`, initial attack side, body `0..2`, and common/rare head `0..2`. Config BODY TYPE 1 writes `+0x24C/+0x24D=3`; FLYBLOWN is independent byte `+0x24E` and has no arm-side draw. Selector 3 uses scale `1.15`, root translation `-8`, body local shift `-5`, and two overlay shifts `-4` from the transformed arm anchors. Idle tick `0x004863A0` advances phases. Renderer `0x00493390` quantizes body rotation to `round(sinDeg(+0x210)*45/10)*10`, head rotation to `round(sinDeg(+0x214*0.5)*20/5)*5 + +0x218`, and draws rear/front arm rotations as `-+0x228` / `+0x22C`. During beat, `+0x23C` adds a side-selected arm swing `round(+0x23C/10)` and body lean `+/- +0x23C/3`; threshold crossings 50 and 100 launch vertical actor offset `+0x240` with velocities in `[-3.5,-3)` and `[-1.5,-1)`, followed by gravity `+0.4` and clamp at ground. |
| Wraith | one list `[2070..2087]`, facing only, at local `(0,+15)` and scale `2` | renderer `0x00496220` issues exactly that opaque white body draw. Tick/action fields do not feed a body alpha, body frame, or body rotation. |
...
| Demon bomb `0x19` | controller selector array `[0,0,0,1,1,1,1,1,0]`, written to actor `+0x2DC` | `Action_Demon_Spit` constructor `0x0044DD40`; tick `0x0044DF00` advances `p += 0.09375 * attack_speed`, calls the DemonBomb event on crossing marker `4`, and completes on the strict boundary `p>8` |
| Demon locomotion/body | controller `19 + pose*18 + facing` plus component banks `1`, `62`, `80`, and `98`, all planted from authored controller points. The controller vertical offset is `-abs(sinDeg(+0x140*0.25))*3`. | fixed tick `0x00487300` advances five cosmetic phase lanes `+0x260..+0x270` by `0.25` modulo the atlas angular period. Renderer `0x00498BA0` uses the global fixed tick for two idle joint angles `2*sinDeg(tick)+(+0x2D4)` and `2*sinDeg(tick)+(+0x2D8)`; constructor `0x00479150` seeds those offsets as `0` and `1`. Bomb pose `+0x2DC==1` overrides them with `+40` and `-40` degrees. |
...
The complete survival-family audit finds no analogous Skeleton-style head-turn
lane elsewhere. Imp's upper phase/alpha, Zombie's articulated head/arms/body,
and Demon's joint lanes have distinct recovered owners; Wraith is facing-only;
Coffin and Maggot are not upper-articulated. The 2026-08-27 Zombie supersession
adds the previously skipped BODY TYPE 3 and body-matrix attachment branches; it
does not add a head-turn field or authorize one for Archer.
...
select another sprite state. Imp upper-effect alpha is likewise local before
the ordinary source-alpha blend; Wraith's body draw remains opaque white in
every live action state. The exact fog/tint/blend
