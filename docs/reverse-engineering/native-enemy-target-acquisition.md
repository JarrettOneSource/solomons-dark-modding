...
slots, round-trips through the exact ActorWorld bucket entry, and passes the
same native region-table comparison as a stock candidate. `Golem (0x7F4)` is
included in the explicit type policy for clarity even though its stock list
