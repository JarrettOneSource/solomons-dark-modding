...
| 74 | **Ether Drain.** Pay `mManaCost[r]`; create `EtherDrain 0x807` with 40-tick scale-in, 1,000 active ticks, and 20-tick scale-out. Nearby actors are pulled inward and close actors receive `mDamage[r]/100` per-tick contact with flags `0x10A`; loose objects are also pulled and consumable nonempty Gold/Sack containers are protected from premature removal. Constructor `0x005F8360` writes `int(0x00820230 * 10)=1000` to `+0x144`; tick `0x0061CF20` decrements it only in phase 1. | HIGH |
