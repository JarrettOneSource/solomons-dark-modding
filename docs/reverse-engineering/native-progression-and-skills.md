...
| 58 | **Meditation.** Refresh writes idle delay `mSeconds[r]*100` to `+0x884`, recovery input `mValue[r]-1` to `+0x890`, and uses `+0x888/+0x88C` for elapsed/ramp. Once idle and not firing, the meditation lane restores `mValue[r] * mana_recovery(+0x98) / game_timing_scale` MP per native tick. Concentration stops walking/acting from clearing the lesser ramp; it does not change the rank arrays. | HIGH |
