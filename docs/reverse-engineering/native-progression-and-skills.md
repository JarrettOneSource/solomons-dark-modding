...
| 10 | **More Missiles.** At Missile cast, `quantity = round(mQuantity[r])`; create zero-based children `i=0..N-1` in native order and add `mManaCost[r]` to cost. With `step = N<4 ? 30deg : 20deg`, `base = aim + (N even ? step/2 : 0)`, and `tier=ceil(i/2)`, heading is `base + (i even ? +1 : -1)*tier*step` (for example `N=4`: `+10,-10,+30,-30`; `N=5`: `0,-20,+20,-40,+40`). One cast-time damage roll is copied unchanged to every child. Visual scale stays `1`; only homing turn input decays by paired tier, as `2*(1+mSpeed/100)*0.75^tier`, while every child speed is `3*(1+mSpeed/100)`. Each child performs its own 100-unit forward-probe acquisition. | HIGH |
