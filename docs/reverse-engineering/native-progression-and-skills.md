...
| 11 | **Call Leviathan.** On secondary cast, pay `mManaCost[r]`, create `Leviathan 0x7F2` at the aimed point, and build `round(mQuantity[r])` appendages (doubled by the compiled Maximize-Leviathan item bit). The actor scales in by `0.025` for 40 ticks, runs for `16 * 100 = 1600` active ticks, then scales out by `0.04` for 25 ticks. Each tracking appendage fires an owned `EtherBolt 0x7F3` carrying `mDamage[r]` after a native `[75,100]`-tick countdown; bolts travel at 10 units/tick, live for 100 ticks, then fade unless contact removes them first. | HIGH |
...
| 16 | **Fireball.** On cast, pay the sum of active rows 16-20, create one `Fireball 0x7D4`, and snapshot rows 16-20/22 plus a private random seed. If Explode is active, accepted direct contact deals `mDamage16[r] - mDamage18[r]`; otherwise it deals `mDamage16[r]`. Removal and the fixed Fireball impact presentation precede the area/ember helper. | HIGH |
...
| 21 | **Ring of Fire.** Pay `mManaCost[r]`; `0x0063F920` creates exactly 30 visual-only `MovingFire 0x7E6` segments at 12-degree base-heading intervals and one damaging `Shockwave 0x7E7`. The Shockwave starts at radius `75`, grows by `6` per tick, carries `mDamage[r]`, applies one-half of that value plus Burn/Dazzle on each actor's first ten-tick contact, and pushes retained contacts on every even tick by normalized radial distance `waveRadius*pushScalar`. The ten-tick query precedes the push, so newly retained targets move on that same even tick. During the final `0.12375` life band the push scalar recurs by float32 `*0.899999976` per tick. Its draw slot does not paint a missile sprite: it submits DeadHawg record 18 to the Arena light field at radius `waveRadius/140`, intensity `pushScalar`, and shadow flag false. The MovingFire children never receive a damage write in this helper. | HIGH |
| 22 | **Burn.** Refresh total authored damage `mDamage[r]` at actor `+0x89C`. Qualifying fire contact attaches `Mod_Burn 0x1B73` for exactly `200` ticks with per-tick damage `mDamage[r]/200`. Same-type merge retains maximum remaining duration and maximum per-tick damage. It has no separate mana cost. | HIGH |
| 23 | **Firewalker.** Toggle actor byte `+0x8DC`; refresh copies `mDamage[r]` and `mDuration[r]` to `+0x894/+0x898` and adds scalar **`mHoard=50` MP** to actor `+0x740`. While active and outside player mode `2`, `PlayerWizard::Tick 0x00548B00` emits one owned `Fire_Goodguy 0x7EE` on every global tick divisible by ten. That branch is not gated by nonzero movement: velocity only controls the random along/perpendicular birth offset. Each patch receives the cached damage, a randomized fraction of the cached duration, and its own scale/contact geometry. The reserve is absolute: `hoarded_mp += 50`, not `maxMP*50%`. | HIGH-LIVE |
...
| 73 | **Fire Wall.** Pay `mManaCost[r]`; build a 300-unit line perpendicular to aim and create eleven owned `Fire_Goodguy 0x7EE` patches at 30-unit intervals, including both endpoints. At distance `d=0,30,...,300` from the first endpoint, multiply constructor scale by `0.8 + 0.6*sin(pi*d/300)`, overwrite life with scalar `7`, then add a random offset with unsigned radius `RandomFloat(10)` and a random unit heading. The shared `Fire_Goodguy` tick subtracts `0.01` per tick, so this overwrite yields **700 ticks**, not the constructor's 200. Each patch carries `mDamage[r]` and applies the existing fire-patch per-second/tick normalization and Burn interaction. Creation requests `ignite` then `fireballhit`; live patches renew `lowfire__loop`. Dispatcher `0x0054CC50`; constants `0x00785D90=150`, `0x00784ED8=30`, `0x007DE810=10`. | HIGH |
| 74 | **Ether Drain.** Pay `mManaCost[r]`; create `EtherDrain 0x807` with 40-tick scale-in, 1,000 active ticks, and 20-tick scale-out. Constructor `0x005F8360` computes the active countdown as `float32(0x00820230) * double(0x007DE810) = 100 * 10 = 1,000`; tick `0x0061CF20` decrements `+0x144` once per fixed tick. Its retained-candidate refresh uses a 1,024 by 819.2 ellipse, then a radius-512 pressure field pulls actors along the normalized inward vector with strength `intensity*1.1*max(0.1,1-distanceSquared/262144)`; flag-`0x400` objects multiply by the falloff again. Radius-20 actors receive `mDamage[r]/100` contact with flags `0x10A`. Loose objects are also pulled; nonempty Gold/Sack containers are protected from premature center capture. Parent art is four additive `BadGuys[75]` galaxy layers plus source-over `BadGuys[38]` shimmer/pulse and the radius-2 random-intensity light. Active RNG creates `SuckCloud` from `Integer(5)==1` (`Integer(3)` with enhanced effects) and independently creates DeadHawg `177..179` `SuckDebris` from `Integer(50)==1`; debris completion creates the additive `BadGuys[36]` capture flare. The actor renews `PlaneCross__Loop`. The dedicated `sounds\crunchdrain` registry entry is associated but its birth-playback callsite remains unresolved. | HIGH gameplay/presentation; MEDIUM audio |
...
| 80 | **Plane Orb.** Runtime-only primary forced by Planewalker. Cast creates `PlaneOrb 0x7EF`, copies caster ownership, and writes per-tick damage `2 * ether_line_effective_rank_sum / 100` from progression `+0x8D0` (IDs `8,10,9,13,14,15,12`). It starts at speed 1.75, lifetime 1000 ticks, visual scale 0.5, acceleration scalar 1, and random maximum scale in `[1,2.5)`; position advances by `(acceleration+1)*velocity`, acceleration recurs by float32 `*0.980000019`, and scale grows by `0.01` before a `0.02` terminal fade. Every sixth authority tick its `2*scale` hostile query applies exactly five stored per-tick payloads. Presentation uses an additive `BadGuys[75]` core and 7-segment rotating ring (15 segments with Enhanced Effects); enhanced mode emits one `BadGuys[11]` mote each active tick. `distortreality` birth audio is confirmed, while the simultaneous `lightningstart` pitch scalar remains unresolved. It has no terrain collision, CFG, learned ranks, or ordinary offer eligibility. | HIGH except audio pitch scalar |
