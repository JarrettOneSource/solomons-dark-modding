...
| ---: | ---: | --- | --- |
| `8` Magic Missile | `0x0053CFE0` | `0x7D3 MagicMissile` | Creates children `i=0..N-1` with `step=N<4?30:20`, `base=aim+(N even?step/2:0)`, tier `ceil(i/2)`, and heading `base+(i even?+1:-1)*tier*step`; all share one cast-time damage roll. Stores each fanned child's own forward-probe target, common speed `3*smartFactor`, paired child turn input `2*smartFactor*0.75^tier`, pierce state, and owner flags. Visual scale remains one. Smart replacement uses current missile position only after a stored handle stops resolving. |
...
| `32` Frost Jet | `0x00543860` | Builds a widening cone whose angle apex is 30 units behind the real strict-range origin, emits `Anim_FrostJetEffect`/`_Over`, queries actor roots through `0x00641B10`, applies damage, pushback, and `Mod_ColdSlow (0x1B69)`. Its learned branches emit `Anim_Hail`, run the Cold Aura circle query, and add Harden armor up to its configured maximum. |
...
| `1004` Water + Air | `0x00541870` | Clips one beam to the expanded Region view and mask-`0x380` terrain, root-tests mask `0x1086` against its authored four-point polygon, and chains unused hostile roots at strict radius 100. It creates the primary beam, source/contact glows, Frost fades, optional chaining particles, and can attach `Mod_ColdSlow` then `Mod_Stun` before contact dispatch. Enemy body radius is not queried. |
